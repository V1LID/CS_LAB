...
double calculateAverageSalary(struct Employee *emps, int count) {
...
int getEmployeesWithSalaryAboveAverage(struct Employee *emps, int count, struct Employee *result) {
    double averageSalary = calculateAverageSalary(emps, count);
    int selectedCount = 0;
...
int main() {
    ...
    struct Employee aboveAverageEmployees[n];
    ...
    if (aboveAverageCount > 0) {
        ...
        printEmployees(aboveAverageEmployees, aboveAverageCount);
    } else {
